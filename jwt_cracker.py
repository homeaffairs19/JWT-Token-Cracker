...
def verify_jwt_with_secret(token, secret, algorithm):
    """ Verifies the JWT with the given secret using PyJWT's decode function. """
    try:
        # Ensure the token signature is properly validated
...
def try_dictionary_attack(token, dictionary_path, algorithm):
    """ Attempts to crack the JWT using a dictionary attack. """
    with open(dictionary_path, 'r') as file:
        for line in file:
            secret = line.strip()  # Clean up the secret (removes newline characters)
            if not secret:
                continue
            print(f"Trying secret from dictionary: {secret}")  # Debug output to show what we're trying
...
def generate_brute_force_combinations(alphabet, max_length):
    """ Generates all possible combinations of a given alphabet up to a max length. """
    for length in range(1, max_length + 1):
        for combination in product(alphabet, repeat=length):
            yield ''.join(combination)  # Yield each combination as a string
...
def try_brute_force_attack(token, alphabet, max_length, algorithm):
    ...
    for secret in generate_brute_force_combinations(alphabet, max_length):
        print(f"Trying brute-force secret: {secret}")  # Debug output to show what we're trying
        if verify_jwt_with_secret(token, secret, algorithm):
            return secret
    return None
...
if __name__ == "__main__":
    ...
    # Step 1: Validate the token's format and algorithm
    ...
    # Use the detected algorithm if it's valid
    algorithm = detected_algorithm

    # Try cracking using dictionary
    if dictionary_path:
